...
export const InputField: React.FC<InputFieldPropsType> = ({
  ...
  if (type === 'textarea') {
    return (
      <div className="relative flex flex-col gap-[7.5px] md:gap-[7px] ">
        ...
        />
        {errors[name] && (
          <p className="error-message absolute bottom-[-24px] md:bottom-[-24px] left-5 text-[12px] font-geologica font-normal leading-[1.25]  text-error ">
            {errors[name]?.message}
          </p>
        )}
      </div>
  ...
  return (
    <div className="relative flex flex-col w-full gap-[7.5px] md:gap-[7px]">
      <label className="font-geologica text-[16px] leading-[1.3] font-medium md:text-subtitleMd text-subtitle">
        {label}
      </label>
      <input
